...
bool waitMessage(uint16_t id, uint16_t timeout_ms = 400);
...
void setup()
{
    ...
    do {
        ...
        m.set_message_id(Ping1dId::FIRMWARE_VERSION);

        // Prepare the buffer and write to device
        m.updateChecksum();
        pingSerial.write(m.msgData, m.msgDataLength());
        delay(2000);
    } while(!waitMessage(Ping1dId::FIRMWARE_VERSION)); // wait for the response
}
...
void loop()
{
    ...
    // Set the request id
    m.set_message_id(requestIds[counter]);
    ...
    if(waitMessage(requestIds[counter])) {
        ...
        // Handle the message
        switch(parser.rxMessage.message_id()) {
...
bool waitMessage(uint16_t id, uint16_t timeout_ms)
{
    ...
    while (millis() < timeout_time) {
        ...
        while(pingSerial.available()) {
            ...
            if (parser.parseByte(pingSerial.read()) == PingParser::NEW_MESSAGE) {
                if (parser.rxMessage.message_id() == id) {
                    return true;
                }
            }
        }
